...
<div class="ebnf">
  <pre>
    <span class="pippo">getCollection</span> ::= <span style="color:green;"> GET COLLECTION </span>(
...
## Semantics
* The `ID₁` token is present, it represents the name of the collection to retrieve.
* The `ID₂` token is present after the `AT`(@) character, it represents the logical name of the database from which to retrieve the collection. Otherwise the collection is retrieved from the Intermediate Results database.
